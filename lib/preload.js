...
function getOpenModalData() {
  function getHref(query) {
    ...
    let anchor = openModal.querySelector(query);
    return anchor ? anchor.href : undefined;
  }
  ...
  if (openModal !== null) {
    ...
    if (openModalData.isOpened) {
      openModalData.tweetLink = getHref('.tweet-timestamp a');
      openModalData.profileLink = getHref('.account-link');
    }
  }
...
function getContextMenuTemplate () {
  return [{
    ...
    label: 'Copy &image',
    click: (model, item, wnd) => wnd.webContents.copyImageAt(model.x, model.y),
...
var contextMenu = shared.CustomMenu(getContextMenuTemplate(), {});
